...
namespace synth
{
	...
	/* FTYPE dVirFreq = dHertz + (dLFOAmplitude > 0 ? dLFOHertz * digSin(dHertz, dTime, dLFOAmplitude) : 0); // This sounds cool as hell too lol
	FTYPE digSin(FTYPE dHertz, FTYPE dTime, FTYPE dAmplitude = 1.0)
	{
		return dHertz * PI * fmod(dTime, dAmplitude / dHertz) - (PI / 2.0);
	}
	*/

	FTYPE digSin(FTYPE dHertz, FTYPE dTime, FTYPE dAmplitude = 1.0, FTYPE dLFOHertz = 0.0)
	{
		return dHertz * PI * fmod(dTime, dAmplitude / dHertz) - (PI / 2.0);
	}
		...
		const FTYPE dLFOHertz = 0.0, const FTYPE dLFOAmplitude = 0.0, FTYPE dCustom = 50.0)
	{
		...
		// FTYPE dFreq = dHertz * 2.0 * pi * dTime + dLFOAmplitude * dHertz * sin(dLFOHertz * 2.0 * PI * dTime);
		// FTYPE dVirFreq = dHertz + (dLFOAmplitude * dHertz * digSin(dLFOHertz, dTime)); // This sounds cool as hell, but is wrong
		// Found this one after the one listed in `OSC_SAW_ANA` - dOutput += (sin(n * dFreq * dTime)) / n;
		// FTYPE dVirFreq = dHertz + (dLFOAmplitude > 0 ? dLFOHertz * digSin(dHertz, dTime, dLFOAmplitude) : 0); // This sounds cool as hell too lol
		FTYPE dVirFreq = dHertz + (dLFOAmplitude > 0 ? dHertz * digSin(dHertz, dTime, dLFOAmplitude, dLFOHertz) : 0);
		...
		case OSC_SAW_ANA: // Saw wave (analogue / warm / slow)
		{
			...
			for (FTYPE n = 1.0; n < dCustom; n++)
				// dOutput += (sin(n * w(dHertz) * dTime)) / n; // original
				// dOutput += (sin(n * dFreq * dTime)) / n; // This sounds cool as hell
		...
		case OSC_SAW_DIG: // Saw Wave (Optimized / harsh / fast)
			...
			// return (2.0 / PI) * digSin(dVirFreq, dTime);
			return (2.0 / PI) * digSin(dHertz, dTime);
